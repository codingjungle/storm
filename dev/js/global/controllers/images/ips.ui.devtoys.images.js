;(function ($, _, undefined) {
    'use strict';
    ips.createModule('ips.ui.devtoys.images', () => {
        /**
    ...
    const _objectDevToysImages = function (elem, options) {
        let ajax = ips.getAjax(),
                    ...
                    if(data.dialog.hasClass('devtoysImages')) {
                        if (!_.isUndefined(data.dialog.attr('data-path'))) {
                            let url = ips.getSetting('baseURL') +
                                'index.php?app=devtoys&module=general&controller=toys&do=delete'
                            ajax({
                                type: 'POST',
            ...
            _convert = (e) => {
                e.preventDefault();
                let url = ips.getSetting('baseURL') +
                        'index.php?app=devtoys&module=general&controller=toys&do=images',
                    url2 = ips.getSetting('baseURL') +
                        'index.php?app=devtoys&module=general&controller=toys&do=download&path=';
                ajax({
                    type: 'POST',
