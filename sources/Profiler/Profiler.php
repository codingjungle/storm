...
use IPS\storm\Shared\HtmlDumper;
use IPS\Theme;
use ReflectionClass;
...
class Profiler extends Singleton
{
    ...
    protected function storm(): array
    {
        ...
        $phpImage = Theme::i()->resource('php.png', 'storm', 'global');
        $phpVer = '<a href="' .
        ...
        $mysql = Theme::i()->resource('mysql.png', 'storm', 'global');
        $ips = Theme::i()->resource('ips.png', 'storm', 'global');
        $buttons = [
            'Info' => [
                $phpVer,
                '<img src="' . $ips . '">' => $ipsVer,
...
//        }
    ...
    protected function extraButtons(): array
    {
        ...
        $clearCaches = '<a href="#" class="stormButtons stormButtons--small" data-ipsstormalert data-ipsstormalert-type="confirm" data-ipsstormalert-msg="This will clear the metadata caches that storm generates." data-ipsstormalert-url="' . (string)Url::internal(
                'app=storm&module=other&controller=proxy&do=clearMetaData',
                'front'
            ) . '" ><i class="fa-solid fa-trash-arrow-up"></i> Clear Storm Caches</a>';
        $debug = '';
        if (Settings::i()->storm_profiler_debug_enabled === true) {
        ...
        $toybox = Tpl::get('profiler.storm.global')->toys();
        return [
            $debug,
            $toybox,
            $generateMeta,
