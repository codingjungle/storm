...
use IPS\storm\Center\Traits\LanguageBuilder;
 ...
 *
 * @package IPS\storm\Center\Sources\Generator
...
abstract class GeneratorAbstract
{
    ...
    final public function process(): void
    {
        ...
        if ($this->type !== 'Group' && $this->type !== 'Member' && in_array($this->type, static::$arDescendent, true)) {
        ...
        if ($this->overrideDir === false) {
            if ($this->type === 'Api') {
                $dir = Application::getRootPath() . '/applications/' . $this->application->directory . '/api/';
            } else {
                $dir = Application::getRootPath(
        ...
        try {
            ...
            if ((bool)$this->scaffolding_create === true && in_array($this->type, static::$arDescendent, false)) {
                ...
                if (is_array($this->scaffolding_type) && in_array('db', $this->scaffolding_type, false)) {
                    try {
                        if ($this->classname_lower !== 'member' && $this->classname_lower !== 'group') {
                            $this->db->add('bits');
                        }
